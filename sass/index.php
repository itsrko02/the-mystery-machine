...
<head>
    ...
    <title>Tesla | Robotics Club</title>
    <!-- Bootstrap Core CSS -->
...
<body id="page-top" data-spy="scroll" data-target=".navbar-fixed-top">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-8 col-md-offset-2">
                        <h1 class="brand-heading">TURING</h1>
                        <p class="intro-text">Solve dandkans adnaknsd asndlandas asdnasnd<br>brought to you by Robotics Club, MMMUT.</p>
    ...
    <section id="register" class="container content-section text-center" style="font-size:20px; padding-top:120px;">
		<div class="row" style=" padding-top:50px; background-image:url('img/downloads-bg.jpg'); border-radius:7px;">
		<h1>Register</h1>
...
{
	if((!empty($_POST["name"])) and (!empty($_POST["email"])) and (!empty($_POST["pwd"])) and (!empty($_POST["college"])) and (!empty($_POST["branch"])) and (!empty($_POST["year"])) and ($_POST["pwd"]==$_POST["cpwd"]) and !empty($_SESSION['captcha_code']) and strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code'])== 0)
	{
		$email=$_POST["email"];
		...
		{
			$q="INSERT INTO users values('".$_POST["name"]."','".$_POST["email"]."','".$password."','".$_POST["college"]."','".$_POST["branch"]."',".$_POST["year"].",0,0,0,0);";
...
<?php
	if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if(isset($_POST['fsubmit']) and !empty($_POST["femail"]))
	{
		$result=mysqli_query($conn,"SELECT * FROM users WHERE email='".$_POST['femail']."';");
		if($result and mysqli_num_rows($result)==1)
		{
			$acode=mt_rand(1000000000,9999999999);
			$result=mysqli_query($conn,"UPDATE users SET actcode='".$acode."' WHERE email='".$_POST['femail']."';");
			if($result)
			{
				mail($_POST["femail"],"Password Reset Link from TURING","Your password reset link is http://turing.in/reset.php?email=".$_POST["femail"]."&actcode=".$acode);
				echo '<script>window.onload=function(){$(\'#mailsent\').modal();}</script>';
			}
			else echo '<script>window.onload=function(){$(\'#fail\').modal();}</script>';
		}
		else echo '<script>window.onload=function(){$(\'#mailnotfound\').modal();}</script>';
	}
}
?>
    <!-- jQuery -->
